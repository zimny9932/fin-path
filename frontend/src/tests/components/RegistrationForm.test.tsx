...
describe("RegistrationForm", () => {
  ...
  it("should render the registration form correctly", () => {
    ...
    expect(
      screen.getByRole("button", { name: /stwórz konto/i })
    ).toBeInTheDocument();
  });
  ...
  it("should display validation errors for empty fields", async () => {
    ...
    expect(
      await screen.findByText(/adres e-mail jest wymagany/i)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/hasło musi mieć co najmniej 8 znaków/i)
    ).toBeInTheDocument();
    expect(
  ...
  it("should display an error message if the email is already taken", async () => {
    ...
    expect(
      await screen.findByText(/ten adres e-mail jest już zajęty/i)
    ).toBeInTheDocument();
  });
});
